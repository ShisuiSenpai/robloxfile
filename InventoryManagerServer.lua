--[[
	...
	⚠️ IMPORTANT: This script MUST load FIRST! 
	   Rename it to "1_InventoryManagerServer" or similar to ensure it loads
	   before CrateSystemServer and MultiSwordSystemServer.
...
-- Structure: {[userId] = {swords = {swordName = true, ...}, equippedSword = "SwordName"}}
...
local function addSwordToInventory(player, swordName)
	...
	if not SwordConfig.Swords[swordName] then
		warn("Attempted to add invalid sword: " .. swordName)
		return false
	end
...
-- Store in ReplicatedStorage so other scripts can access
local managerValue = Instance.new("ObjectValue")
managerValue.Name = "InventoryManager"
managerValue.Value = script
managerValue.Parent = ReplicatedStorage

-- Attach API to script for requiring
script:SetAttribute("APIReady", true)
