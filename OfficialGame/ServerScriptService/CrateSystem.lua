...
local function switchPlayerSword(player, swordName)
	...
	if not swordConfig then
		warn("[CRATE] Sword config not found: " .. swordName)
		return false
	end
...
proximityPrompt.Triggered:Connect(function(player)
	...
	print("[CRATE] " .. player.Name .. " is opening a crate! Chosen sword: " .. chosenSword)
...
switchSwordEvent.OnServerEvent:Connect(function(player, swordName)
	...
	if not SwordConfig.Swords[swordName] then
		warn("[CRATE] Invalid sword switch attempt: " .. tostring(swordName))
		return
	end
	...
	print("[CRATE] Switched " .. player.Name .. " to " .. swordName)
end)
