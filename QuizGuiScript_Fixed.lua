-- QuizGui.client.lua
-- Client-side script for the new white & blue quiz UI (FIXED SCALING)
...
-- FIXED: Remove the UIScale approach and use proper positioning instead
local function setupResponsivePositioning()
	-- Store original positions as they are in the UI hierarchy
	local originalPositions = {
		questionFrame = UDim2.new(0.5, -400, 0.55, 0),
		timerFrame = UDim2.new(0.5, -100, 0.05, 0),
		answerA = UDim2.new(0.5, -390, 0.72, 0),
		answerB = UDim2.new(0.5, 10, 0.72, 0),
		answerC = UDim2.new(0.5, -390, 0.72, 85),
		answerD = UDim2.new(0.5, 10, 0.72, 85),
		nextQuestionFrame = UDim2.new(0.349, 0, 0.85, 0)
	}
	
	-- Return the positions for use in animations
	return originalPositions
end

-- Get stored positions
local POSITIONS = setupResponsivePositioning()
...
local function animateIn()
	...
	-- FIXED: Use the stored positions instead of hardcoded ones
	...
	-- Animate question to its proper position
	local questionTween = TweenService:Create(questionFrame, TweenInfo.new(0.8, Enum.EasingStyle.Back), {
		Position = POSITIONS.questionFrame
	})
	questionTween:Play()

	-- Animate timer to its proper position
	local timerTween = TweenService:Create(timerFrame, TweenInfo.new(0.6, Enum.EasingStyle.Back), {
		Position = POSITIONS.timerFrame
	})
	timerTween:Play()

	-- Animate answer buttons to their proper positions
	task.wait(0.2)
...
function ShowQuestion(question, totalTime)
	...
	-- FIXED: Reset answer frames to start below screen using stored positions
	local originalPositions = {
		POSITIONS.answerA,
		POSITIONS.answerB,
		POSITIONS.answerC,
	...
	for i, answerFrame in ipairs(answerFrames) do
		...
		-- Start below screen but keep X position from stored values
...
-- Initially hide
gui.Enabled = false
print("[QuizUI] Client script loaded - Fixed scaling")